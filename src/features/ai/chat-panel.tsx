...
type AiWorkbenchTab = "chat" | "diagnosis";

const AI_WORKSPACE_TABS: Array<{
  id: AiWorkbenchTab;
  label: string;
  caption: string;
...
}> = [
  {
    id: "chat",
    label: "AI 对话",
    caption: "流式会话、工具轨迹与报告协作",
    icon: Sparkles,
  },
  {
    id: "diagnosis",
    label: "AI 诊断",
    caption: "分钟级告警轮询与一键诊断报告",
    icon: Activity,
  },
];
...
export function ChatPanel() {
  ...
  const [activeTab, setActiveTab] = useState<AiWorkbenchTab>("chat");
  ...
  const handleTabChange = (nextTab: AiWorkbenchTab) => {
    startTransition(() => {
  ...
  return (
    ...
    >
      ...
      <PageHeader
        ...
        title="AI 助手"
        description="把对话协作、统计报告和 AI 诊断收拢到一个工作台中。你可以在同一页里切换流式会话与分钟级诊断视角，先读信号，再决定是否触发更重的分析。"
        actions={
          <>
      ...
      >
        <div className="flex flex-wrap items-center gap-3">
          <div
            role="tablist"
            aria-label="AI 工作台切换"
            className="inline-flex flex-wrap items-center gap-2 rounded-[1.4rem] border border-border/70 bg-background/80 p-2"
          >
            {AI_WORKSPACE_TABS.map(({ id, label, caption, icon: Icon }) => {
              const isActive = activeTab === id;

              return (
                <button
                  key={id}
                  id={`ai-workbench-tab-${id}`}
                  type="button"
                  role="tab"
                  aria-selected={isActive}
                  aria-controls={`ai-workbench-panel-${id}`}
                  onClick={() => handleTabChange(id)}
                  className={cn(
                    "rounded-[1.1rem] border px-4 py-3 text-left transition-all",
                    isActive
                      ? "border-primary/20 bg-primary text-primary-foreground shadow-[0_18px_48px_-28px_var(--color-primary)]"
                      : "border-transparent bg-transparent text-muted-foreground hover:border-primary/15 hover:bg-primary/5 hover:text-foreground",
                  )}
                >
                  ...
                  <p
                    className={cn(
                      "mt-1 text-xs",
                      isActive ? "text-primary-foreground/82" : "text-muted-foreground",
                    )}
                  >
                    {caption}
                  </p>
